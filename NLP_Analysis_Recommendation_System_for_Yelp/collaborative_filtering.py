...
def data_prep():
    sql = """
        ...
        and user_id in (select user_id from parsed_review group by user_id having count(user_id)>10)
    """
    user_review = pd.read_sql(sql,conn)
    return user_review

all_user_id = pd.read_sql("select user_id from parsed_review group by user_id having count(user_id)>10",conn)
...
#load model
svd_load = pickle.load(open('../Models/Collaborative_filtering.model', 'rb'))
...
def fit_model(data):
    ...
    # save model
    path = '../Models/Collaborative_filtering.model'
...
# a = Collaborative_filtering(user_id,n=3,address=address, dist_limit=15)
